...
if not exist "!temp!\MalEx" mkdir "!temp!\MalEx"
if exist !logfile! del !logfile!
call :logger "Logging start ----------"
echo.
echo           ~~~ Malware Execution Automation Script [Obfuscated v1.00] ~~~
...
for %%f in ( !extensions2! ) do move /y "%%f" "!destinationdir!" >nul 2>&1
call :logger "Import completed."
cd /d "!destinationdir!" 2>nul
set /a count=0
set "category=!extensions2!"
for %%f in ( !extensions2! ) do set /a count+=1
...
:continue3
set /a count0=0
for %%f in ( !category! ) do set /a count0+=1
if !count0! equ 0 (
goto :empty
) else (
goto :continue4
...
set "filledbar="
for /l %%a in (1,1,!filled!) do set "filledbar=!filledbar!="
set "blankbar="
for /l %%a in (1,1,!blank!) do set "blankbar=!blankbar! "
echo  Progress:
...
set /a count=0
set /a count2=0
for %%f in ( !category2! ) do set /a count0+=1
...
for /f %%a in ('type !urlfile! ^| find /i /c "http"') do set urls=%%a
...
if exist !urlfile2! del !urlfile2!
set /a count=0
...
echo.
echo All MalEx commands run with administrative privileges to avoid frequent UAC warnings ^(especially in Malware test^). As a result, malware will execute with highest privileges allowing them to perform a critical damage to your data. So use this script AT YOUR OWN RISK. The author hold no responsibility for any damage or data-theft caused by its usage.)>!helpfile!
(echo Set objFSO = CreateObject^("Scripting.FileSystemObject"^)
...
echo Set objFSO = Nothing)>!vbsfile3!
